...
VERIBLE_PARSER_SEQ = re.compile('\033\\[.*?m')


def verible_parser_colorize(formats: List[str], strings: List[str]) -> str:
    result = ''
    fi = 0
    for s in strings:
...
class _TreeIteratorBase:
    ...
    def _iter_tree(self, tree: Optional['Node']) -> Iterable['Node']:
        raise NotImplementedError(
            "Subclass must implement '_iter_tree' method")
...
class Node(anytree.NodeMixin):
    """Base VeribleParser syntax tree node.
...
class BranchNode(Node):
    ...
    def find(self, filter_: Union[CallableFilter, KeyValueFilter, None],
        ...
        """
        return next(
            self.iter_find_all(filter_, max_count=1, iter_=iter_, **kwargs),
            None)
    ...
    def to_formatted_string(self) -> str:
        ...
        return verible_parser_colorize(['37', '1;97'], ["[", tag, "]"])
...
class LeafNode(Node):
    ...
    def to_formatted_string(self) -> str:
        return verible_parser_colorize(['90'], ['null'])
...
class TokenNode(LeafNode):
    ...
    def to_formatted_string(self) -> str:
        tag = self.tag if self.tag == repr(self.tag)[1:-1] else repr(self.tag)
        parts = [
            verible_parser_colorize(['37', '1;97'], ["[", tag, "]"]),
            verible_parser_colorize(
                ['33', '93'],
                ['@(', str(self.start), "-",
                 str(self.end), ")"]),
        ]
...
class VeribleParser:
    ...
    Args:
      exec_path: path to ``verible-verilog-syntax`` binary.
    """
    def __init__(self, exec_path: str = 'verible-verilog-syntax'):
        self.exec_path = exec_path

    @staticmethod
    def _transform_tree(tree, data: SyntaxData,
                        skip_null: bool) -> Optional[RootNode]:
        def transform(tree):
            if tree is None:
                return None
            if "children" in tree:
        ...
        children = [
            transform(child) or LeafNode() for child in tree["children"]
            if not (skip_null and child is None)
        ]
        tag = tree["tag"]
    ...
    def _parse(
            ...
            options: Optional[Dict[str, Any]] = None) -> Dict[str, SyntaxData]:
        ...
        proc = subprocess.run([self.exec_path, *args, *paths],
        ...
        for file_path, file_json in json_data.items():
            ...
            if file_path == '-':
                ...
                with open(file_path, 'rb') as fp:
                    file_data.source_code = fp.read()

            if 'tree' in file_json:
                file_data.tree = VeribleParser._transform_tree(
                    file_json['tree'], file_data, options['skip_null'])

            if 'tokens' in file_json:
                file_data.tokens = VeribleParser._transform_tokens(
                    file_json['tokens'], file_data)

            if 'rawtokens' in file_json:
                file_data.rawtokens = VeribleParser._transform_tokens(
                    file_json['rawtokens'], file_data)

            if 'errors' in file_json:
                file_data.errors = VeribleParser._transform_errors(
                    file_json['errors'])
